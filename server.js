...
io.on("connection", function (socket) {
    ...
    socket.on("submitName", function (nickName) {
        ...
        if (val == 0 || val == 3) {
            ...
            var newPlayer = { id: socket.id, nickName: filter.clean(nickName), room: DATA.SETTINGS.defaultRoom };
            ...
            var roomState = getRoomState(newPlayer.room);
            //send the player the state of the room, current players and such
            socket.emit("joinedRoom", newPlayer.room, roomState);
    ...
    socket.on("disconnect", function () {
        try {
            ...
            if (disconnectingPlayer != null) {
                io.sockets.emit("playerLeft", { id: socket.id, nickName: disconnectingPlayer.nickName, room: disconnectingPlayer.room, disconnect: true });
                ...
                if (gameState.players[socket.id].admin && obj.message.charAt(0) == "/") {
                    ...
                    if (filter.isProfane(test) || filter.isProfane(test2) || filter.isProfane(test3) || test4 == "") {
                        ...
                        if (obj.message != "")
                            ...
                            io.to(obj.room).emit("playerTalked", { id: socket.id, nickName: gameState.players[socket.id].nickName, message: obj.message });
                    }
                }
    ...
    socket.on("changeRoom", function (obj) {
        try {
            ...
            if (roomPlayers > MAX_PLAYERS_PER_ROOM && MAX_PLAYERS_PER_ROOM != -1) {
                ...
                socket.emit("errorMessage", "The room looks full");
            }
            else {
                console.log("Player " + socket.id + " moved from " + obj.from + " to " + obj.to);

                //same for first joining, sending everybody in the room the player state
                var movingPlayer = gameState.players[socket.id];
                movingPlayer.room = obj.to;
                ...
                io.to(obj.from).emit("playerLeft", { id: socket.id, nickName: movingPlayer.nickName, room: obj.from, disconnect: false });

                //send all OTHER players information about the new player
                //upon creation destination and position are the same 
                socket.broadcast.to(movingPlayer.room).emit('playerJoined', movingPlayer);

                //retrieving the state of the current room
                var roomState = getRoomState(movingPlayer.room);
                //send ONLY the new player the state of the room, current players and such
                socket.emit("joinedRoom", movingPlayer.room, roomState);
            }
        } catch (e) {
...
//in this case the room state is just a list of players in the room but in more complex app it would include the persistent states of various objects
function getRoomState(roomId) {
    var roomState = {};
    roomState.players = {};
    print("Get state for " + roomId);

    //create a subset of players currently in this room
    for (var id in gameState.players) {

        if (gameState.players[id].room == roomId) {
            console.log(gameState.players[id].nickName + " is in the room");
            roomState.players[id] = { id: id, nickName: gameState.players[id].nickName, room: roomId };
        }
